...
The Playground lets you test Thea's answers before members ever see them. Ask questions in a full conversation interface inside the dashboard — Thea responds exactly as it would in Discord, using your live knowledge base, but nothing is posted to any channel. Use it to verify that new sources are being picked up, to understand why certain questions escalate, and to tune your confidence threshold with real data.
...
<Steps>
  <Step title="Add or update a source">
    Go to **Settings → Knowledge → Sources** and add your new source or trigger a re-sync on an existing one. Wait for indexing to complete.
  </Step>
